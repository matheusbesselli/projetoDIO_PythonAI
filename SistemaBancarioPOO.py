...
class Cliente:
    ...
    def fazerTransacao(self, conta, transacao):
...
class Conta:
    ...
    @classmethod
    def registrarNovaConta(cls, cliente, numero):
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        ...
        excedeuSaques = numero_saques >= self._limiteSaques
...
def filtrarNovoCliente(cpf, clientes):
    clientes_filtrados = [cliente for cliente in clientes if cliente.cpf == cpf]
    return clientes_filtrados[0] if clientes_filtrados else None


def recuperarContadoCliente(cliente):
...
def depositar(clientes):
    ...
    cliente = filtrarNovoCliente(cpf, clientes)
    ...
    conta = recuperarContadoCliente(cliente)
    if not conta:
        return

    cliente.fazerTransacao(conta, transacao)
...
def criarConta(numero_conta, clientes, contas):
    ...
    conta = ContaCorrente.registrarNovaConta(cliente=cliente, numero=numero_conta)
    contas.append(conta)
